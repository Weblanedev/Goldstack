...
const Navbar = () => {
	...
	return (
		<>
			<Disclosure
				as='nav'
				className={`w-full py-2 relative z-50
				${location.pathname === "/" ? "bg-[#011b33] lg:bg-transparent" : "bg-[#011b33]"}
				text-white
			`}
			>
				{({ open, close }) => (
						...
						>
							<div className='relative flex items-center justify-between h-16 py-2'>
								...
								>
									...
									'
									>
										<NavLink to='/' onClick={() => close()} className="flex gap-[10px] items-center">
											<img src={logo} alt='Goldstack logo' className='w-[50px] sm:w-[75px] sm:h-[37.5px]'/>
											<p className='text-[20px] sm:text-[28px] font-bold'>GoldStack</p>
										</NavLink>
									</div>
									<div
										// sm:mx-auto
